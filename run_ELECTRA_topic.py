...
from transformers import (BertConfig, BertTokenizer,
                          ElectraConfig, ElectraTokenizer, RobertaConfig, RobertaTokenizer)

from modeling import (BaselineGRU, BaselineTopic, ElectraForMultipleChoicePlus, Baseline, BertBaseline, RobertaBaseline,
                      BertForMultipleChoicePlus, RobertaForMultipleChoicePlus)
from utils import get_topicid_bypos
...
class InputFeatures(object):
    ...
    def __init__(self, example_id, choices_features, label):
        ...
        self.choices_features = [
            ...
            for input_ids, input_mask, segment_ids, sep_pos, turn_ids, topic_ids, topic_mask in choices_features
        ]
        self.label = label
...
def convert_examples_to_features(examples, label_list, max_seq_length, max_utterance_num,
                                 tokenizer, output_mode, max_topic_length):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for ending_idx, (text_a, text_b) in enumerate(zip(example.text_a, example.text_b)):
            ...
            for toks in tokens_b_raw.split("[SEP]")[-max_utterance_num - 1:-1]:
                tmp_tokens = toks.split()
                topic_ids.extend(get_topicid_bypos(tmp_tokens, current_pos))
                context_len.append(len(tmp_tokens) + 1)
                tokens_b.extend(tmp_tokens)
                tokens_b.extend(["[SEP]"])
            ...
            segment_ids = [0] * (len(tokens)) # for context/question

            tokens_a += ["[SEP]"]
            tokens += tokens_a

            topic_ids.extend(get_topicid_bypos(tokens_a, current_pos))

            if(len(topic_ids) > tmp_max_topic_length):
                tmp_max_topic_length = len(topic_ids)

            segment_ids += [1] * (len(tokens_a)) # for response
            
            turn_ids += [len(sep_pos)] * len(tokens_a) 
            sep_pos.append(len(tokens) - 1)

            input_ids = tokenizer.convert_tokens_to_ids(tokens)
            #topic_ids = tokenizer.convert_tokens_to_ids(topic_tokens)
            ...
            topic_padding = [0] * (max_topic_length - len(topic_ids))
            topic_mask = [1] * len(topic_ids)
            topic_mask += topic_padding
            topic_ids += topic_padding
            ...
            assert len(topic_ids) == max_topic_length
            assert len(topic_mask) == max_topic_length

            choices_features.append((input_ids, input_mask, segment_ids, sep_pos, turn_ids, topic_ids, topic_mask))
        ...
        #     for choice_idx, (input_ids, input_mask, segment_ids, sep_pos, turn_ids, topic_ids, topic_mask) in enumerate(choices_features):
...
def main():
    ...
    parser.add_argument("--max_topic_length",
                        default=80,
                        type=int,
                        help="max length of topic words")
    parser.add_argument("--do_topic",
                        action='store_true',
                        help="Whether to use topic.")
    parser.add_argument("--do_gru",
                        action='store_true',
                        help="Whether to use topic.")
    ...
    if args.baseline:
        if args.model_type == 'electra':
            ...
            if args.do_topic:
                model_class = BaselineTopic
            elif args.do_gru:
                model_class = BaselineGRU
    ...
    if args.do_train:
        ...
        try:
            ...
            train_features = convert_examples_to_features(
                train_examples, label_list, args.max_seq_length, args.max_utterance_num, tokenizer, output_mode, args.max_topic_length)
        ...
        all_topic_ids = torch.tensor(select_field(train_features, 'topic_ids'), dtype = torch.long)
        all_topic_mask = torch.tensor(select_field(train_features, 'topic_mask'), dtype=torch.long)
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sep_pos, all_turn_ids, all_label_ids, all_topic_ids, all_topic_mask)
        ...
        try:
            ...
            eval_features = convert_examples_to_features(
                eval_examples, label_list, args.max_seq_length, args.max_utterance_num, tokenizer, output_mode, args.max_topic_length)
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:
        ...
        all_topic_ids = torch.tensor(select_field(eval_features, 'topic_ids'), dtype=torch.long)
        all_topic_mask = torch.tensor(select_field(eval_features, 'topic_mask'), dtype=torch.long)
        ...
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sep_pos, all_turn_ids, all_label_ids, all_topic_ids,all_topic_mask)
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    if args.fp16:
                        ...
                        # if args.ffp16 is False, BertAdam is used that handles this automatically
