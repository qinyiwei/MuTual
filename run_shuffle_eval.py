...
from modeling import (ElectraForMultipleChoicePlus, Baseline, BertBaseline, RobertaBaseline, BertForMultipleChoicePlus,
...
def convert_examples_to_features(examples, label_list, max_seq_length, max_utterance_num,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for ending_idx, (text_a, text_b) in enumerate(zip(example.text_a, example.text_b)):
            ...
            tokens_b_raw_split = np.asarray(tokens_b_raw.split("[SEP]"))
            even = []
            odd = []
            for i in range(len(tokens_b_raw_split)):
                if i % 2 == 0:
                    even.append(i)
                else:
                    odd.append(i)
            ...
            for i in range(len(tokens_b_raw_split)):
                if i %2 ==0:
                    shuffle_sequence.append(even[int(i/2)])
                else:
                    shuffle_sequence.append(odd[int((i-1)/2)])

            tokens_b_raw_split = tokens_b_raw_split[shuffle_sequence]
            for toks in tokens_b_raw_split[-max_utterance_num - 1:-1]:
...
def compute_metrics(task_name, preds, labels):
    ...
    if task_name in ["ubuntu", "ecd"]:
        return {"acc": simple_accuracy(preds_class, labels), "recall@10": ComputeR10(preds_logits, labels),
                "recall@2": ComputeR2_1(preds_logits, labels), "DAM": evaluate(preds_logits, labels)}
...
def main():
    ...
    if args.do_eval:
        ...
        cached_train_features_file = args.data_dir + '_{0}_{1}_{2}_{3}_{4}_{5}'.format(
            list(filter(None, args.model_name_or_path.split('/'))).pop(), "valid", str(args.task_name),
        ...
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sep_pos, all_turn_ids,
                                  all_label_ids)
